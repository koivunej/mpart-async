    ...
    E: StdError + 'static,
{
    ...
    pub fn content_type<'a>(&'a self) -> Result<&'a str, MultipartError<E>> {
    ...
    pub fn filename<'a>(&'a self) -> Result<&'a str, MultipartError<E>> {
    ...
    pub fn name<'a>(&'a self) -> Result<&'a str, MultipartError<E>> {
        if let Some(val) = self.headers.get("content-disposition") {
            let string_val = val.to_str().map_err(|_| MultipartError::InvalidHeader)?;
...
where
    ...
    E: StdError + 'static,
{
    type Item = Result<MultipartField<S, E>, MultipartError<E>>;
...
#[derive(Error, Debug)]
pub enum MultipartError<E: StdError + 'static> {
    ...
    E: StdError + 'static,
{
    ...
    fn return_poll(&mut self) -> Poll<Option<Result<ParseOutput, MultipartError<E>>>> {
...
fn get_headers<E: StdError + 'static>(
    buffer: &[u8],
) -> Result<HeaderMap<HeaderValue>, MultipartError<E>> {
...
where
    ...
    E: StdError,
{
    type Item = Result<ParseOutput, MultipartError<E>>;

    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        use futures::stream::StreamExt;
        let self_mut = &mut self.as_mut();

        // FIXME: it's too early to poll the inner stream
                ...
                State::ReadingBoundary => {
                    ...
                    if self_mut.buffer.len() < boundary_len + 4 {
                        match futures::ready!(self_mut.stream.poll_next_unpin(cx)) {
                            ...
                            Some(Err(e)) => {
                                return Poll::Ready(Some(Err(MultipartError::Stream(e))))
                            }
                            None => {
                        ...
                        && &self_mut.buffer[boundary_len + 2..boundary_len + 4] == b"\r\n"
                    {
                        ...
                        println!(
                            "{:>12}: {}\n\
                             {:>12}: {}",
                            "boundary",
                            DefaultEscaper(&self_mut.buffer[..boundary_len + 4]),
                            "rest",
                            DefaultEscaper(&self_mut.buffer[boundary_len + 4..]),
                        );
                    ...
                    if let Some(end) = find_bytes(&self_mut.buffer, b"\r\n\r\n") {
                        ...
                        println!(
                            "{:>12}: {}\n\
                             {:>12}: {}",
                            "headers",
                            DefaultEscaper(&self_mut.buffer[..end]),
                            "rest",
                            DefaultEscaper(&self_mut.buffer[end..]),
                        );
                    ...
                    if let Some(idx) = find_bytes(&self_mut.buffer, b"\r") {
                        println!(
                            "{:>12}: {}\n\
                             {:>12}: {}",
                            "body",
                            DefaultEscaper(&self_mut.buffer[..idx]),
                            "trailer",
                            DefaultEscaper(&self_mut.buffer[idx..]),
                        );
                        ...
                        if self_mut.buffer.len() < idx + 6 + boundary_len {
                            ...
                            println!(
                                "exhausted as buffered {} but ending would need {} + 6 + {} == {}",
                                self_mut.buffer.len(),
                                idx,
                                boundary_len,
                                idx + 6 + boundary_len
